...
.ros-card{background:var(--color-surface);border-radius:var(--radius-lg);box-shadow:var(--shadow-sm);padding:18px;}
.ros-card h4{display:flex;align-items:center;gap:9px;margin:0 0 14px;font-size:14px;letter-spacing:.02em;}
.ros-verdict{position:relative;border-radius:var(--radius-lg);padding:22px 24px;overflow:hidden;}
...
def render_brand():
    ...
    _md(
        f"""
        <div style="display:flex;align-items:center;gap:16px;padding:4px 4px 4px 0;">
...
def _gauge_html(icon_name, label, pct, value_str, unit_label, suspicious, verdict_text, sub_text, pts):
    ...
    return f"""
        <div class="ros-card" style="display:flex;flex-direction:column;gap:12px;">
...
def render_agent_gauges(vision: dict, audio: dict, linguistics: dict):
    ...
    _md(
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;">{v_html}{a_html}{l_html}</div>
        """
    )
...
def render_transcript(linguistics: dict):
    ...
    _md(
        ...
        <section class="ros-card" style="display:flex;flex-direction:column;">
          ...
          <div style="display:flex;gap:14px;margin-top:14px;padding-top:12px;border-top:1px solid var(--color-divider);">
...
def render_source_card_open(filename: str, size_mb: float):
    _md(
        f"""
        <section class="ros-card" style="padding:15px 18px;">
          <div style="display:flex;align-items:center;gap:9px;margin-bottom:12px;">
            {icon('movie', 17, 'var(--color-accent)')}
            <h4 style="margin:0;font-size:14px;letter-spacing:.02em;">Source Response</h4>
          </div>
          <div style="display:flex;align-items:center;gap:12px;padding:12px;border:1px solid var(--color-divider);border-radius:var(--radius-md);background:var(--color-bg);">
...
def render_idle_agent_cards():
    ...
    _md(
        f'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:22px;">{cards}</div>'
    )
